...
const parseError = (words: string[], error: any): string => {
    if (error.kind === 'BadWordCount') {
        ...
        return c('Wallet setup').t`Wallet seed phrase checksum is invalid. Please check your last word.`;
                ...
                if (created) {
                    ...
                    if (shouldDoBve && firstAddress && !hasAlreadyLinkedFirstEmail) {
                        try {
                            ...
                            const { DerivationPath, PoolSize, ScriptType, LastUsedIndex } = created.Data;
